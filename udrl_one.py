...
class Behavior(nn.Module):
    ...
    def get_gnn_embedding(self, g, inputs):

        # Graph input to get embedding
        graph = self.conv1(g, inputs)
        graph = torch.sigmoid(graph)
        graph = self.conv2(g, graph)

        return torch.sigmoid(graph)

    def forward(self, gnn_embeddings, command):
        # Encode the the command values
        command = self.command_fn(command)

        # calculate product of command and graph-embedding
        product = torch.mul(gnn_embeddings, command)

        # get the output for the cmobination
        return self.output_nn(product)
    
    def action(self, state, command):
        
        G, node_inputs, leaf_nodes = state
        gnn_embeddings = self.get_gnn_embedding(G.to(device), node_inputs.to(device))
